...
export function isActionable(candidate: GuidanceCandidate): boolean {
  ...
  return (ACTIONABLE.test(text) || CONTEXTUAL_ACTIONABLE.test(text) || SOURCE_REVIEW_ACTIONABLE.test(text) || /^the overall color needs to\b/i.test(text) || /^only provide\b/i.test(text)) && !/^Resources?\b/i.test(text);
}
