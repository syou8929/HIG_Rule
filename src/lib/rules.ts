...
const SHAREPLAY_SOURCE_REVIEW_ACTIONABLE = /^(Briefly describe each activity|When possible, defer app tasks that might delay a shared activity|Smoothly update a shared activity when new participants join)$/i;
...
export function isActionable(candidate: GuidanceCandidate): boolean {
  ...
  return (ACTIONABLE.test(text) || CONTEXTUAL_ACTIONABLE.test(text) || SOURCE_REVIEW_ACTIONABLE.test(text) || SHAREPLAY_SOURCE_REVIEW_ACTIONABLE.test(text) || /^the overall color needs to\b/i.test(text) || /^only provide\b/i.test(text)) && !/^Resources?\b/i.test(text);
}
