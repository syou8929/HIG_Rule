...
const PAGE_SOURCE_REVIEW_ACTIONABLE = /^(Briefly describe each activity|When possible, defer app tasks that might delay a shared activity|Smoothly update a shared activity when new participants join|Stop recording as soon as possible|Request microphone access before using it for recognition|Explain why you request microphone access|Record only as long as needed for the recognition sample)$/i;
